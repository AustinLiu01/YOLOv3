...
#非极大值预测，要在predict的时候才用到，train的时候不用
...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, nms_thresh=0.5):
    ...
    #要滤去小于0.5得分的，证明很大程度上没有，返回的是boolean值
    mask = tf.greater_equal(score, tf.constant(score_thresh))
    # Step 2: Do non_max_suppression for each class
    #对每一个class单独做NMS
    for i in range(num_classes):
        # Step 3: Apply the mask to scores, boxes and pick them out
        #留下的都是认为框里有东西的框的坐标参数和score
        filter_boxes = tf.boolean_mask(boxes, mask[:,i])
        filter_score = tf.boolean_mask(score[:,i], mask[:,i])
        #NMS算法去掉IOU大于阈值的框，代表应该是重复检测了，剩不超过50（default设置）个框
        nms_indices = tf.image.non_max_suppression(boxes=filter_boxes,
                                                   scores=filter_score,
                                                   max_output_size=max_boxes,
                                                   iou_threshold=nms_thresh, name='nms_indices')
        #将剩下的score提取出来，我们认为最后要呈现的用1代表做label
        #boxes_list和score_list都是存有有效boxes和score的
